#!/usr/bin/env python3
"""
Public API for codex-cli-docs skill.
...
Codex CLI documentation management system. This API abstracts away
implementation details and provides simple functions for common operations.

Usage:
    from codex_docs_api import find_document, resolve_doc_id, get_docs_by_tag

    # Find documents by query
    docs = find_document("sandbox configuration")

    # Resolve doc_id to metadata
    doc = resolve_doc_id("codex-cli-getting-started")
...
class CodexDocsAPI:
    ...
    Provides high-level functions for Codex CLI documentation operations.
    ...
    def find_document(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        Args:
            query: Natural language search query (e.g., "how to use sandbox")
        ...
        Example:
            >>> api = CodexDocsAPI()
            >>> docs = api.find_document("sandbox configuration")
            >>> print(docs[0]['title'])
        """
        try:
    ...
    def resolve_doc_id(self, doc_id: str) -> dict[str, Any] | None:
        ...
        Args:
            doc_id: Document identifier (e.g., "codex-cli-getting-started")
        ...
        Example:
            >>> api = CodexDocsAPI()
            >>> doc = api.resolve_doc_id("codex-cli-getting-started")
            >>> print(doc['title'])
        """
        try:
    ...
    def get_docs_by_tag(self, tag: str, limit: int = 100) -> list[dict[str, Any]]:
        ...
        Args:
            tag: Tag to filter by (e.g., "cli", "sandbox", "agents")
    ...
    def get_docs_by_category(self, category: str, limit: int = 100) -> list[dict[str, Any]]:
        ...
        Args:
            category: Category to filter by (e.g., "guides", "api", "examples")
            limit: Maximum number of results to return (default: 100)
        ...
        Example:
            >>> api = CodexDocsAPI()
            >>> docs = api.get_docs_by_category("guides")
            >>> print(len(docs))
        """
        try:
    ...
    def search_by_keywords(self, keywords: list[str], limit: int = 25) -> list[dict[str, Any]]:
        ...
        Example:
            >>> api = CodexDocsAPI()
            >>> docs = api.search_by_keywords(["sandbox", "tools"])
            >>> print(docs[0]['title'])
        """
        try:
    ...
    def get_document_section(self, doc_id: str, section_heading: str) -> dict[str, Any] | None:
        ...
        Example:
            >>> api = CodexDocsAPI()
            >>> section = api.get_document_section("codex-cli-overview", "Installation")
