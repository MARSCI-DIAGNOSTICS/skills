...
def get_http_user_agent() -> str:
    ...
    return get_default('http', 'user_agent', "Codex-Docs-Scraper/1.0 (Educational purposes)")

def get_management_user_agent() -> str:
    """Get user agent string for management/cleanup operations."""
    return get_default('user_agents', 'management', "Codex-Docs-Management-Bot/1.0")

def get_scraper_user_agent() -> str:
    """Get user agent string for scraping operations."""
    return get_default('user_agents', 'scraper', "Codex-Docs-Scraper/1.0 (Educational purposes)")
...
def get_domain_weight(domain: str) -> float:
    ...
    Args:
        domain: Domain name (e.g., "developers.openai.com")
    ...
    # Try to match by category path (e.g., openai.com/codex)
...
def get_query_stop_words() -> set[str]:
    ...
    Example: "when to use codex" → search for "codex" only
...
def get_domain_stop_words() -> set[str]:
    ...
    be filtered from searches (e.g., codex, openai).
