...
# -*- coding: utf-8 -*-
"""
logging_utils.py - Common logging and observability utilities for codex-cli-docs scripts
...
for all scripts in the codex-cli-docs skill.
...
# Changed from CLAUDE_DOCS_ to CODEX_DOCS_
RUN_ID_ENV_VAR = "CODEX_DOCS_RUN_ID"
SOURCE_NAME_ENV_VAR = "CODEX_DOCS_SOURCE_NAME"
...
# CODEX_DOCS_LOG_TO_FILE: "true" to enable file logging
# CODEX_DOCS_LOG_LEVEL: DEBUG, INFO, WARNING, ERROR
# CODEX_DOCS_LOG_JSON: "true" for JSON format
...
def _get_log_level() -> int:
    ...
    # Check env var first
    env_level = os.environ.get("CODEX_DOCS_LOG_LEVEL", "").upper()
    if env_level:
...
class ScriptLogger:
    ...
    def __init__(
        ...
        self._enable_file_logging = (
            ...
            else _get_config_bool("logging", "enable_file_logging", "CODEX_DOCS_LOG_TO_FILE", False)
        )
        self._json_format = (
            json_format if json_format is not None
            else _get_config_bool("logging", "json_format", "CODEX_DOCS_LOG_JSON", False)
        )
        self._write_diagnostics = _get_runtime_value("diagnostics", "write_diagnostics_files", False)
        # HTTP timing: env var CODEX_DOCS_HTTP_TIMING overrides config (for --with-timing flag)
        self._track_http = _get_config_bool("diagnostics", "track_http_timings", "CODEX_DOCS_HTTP_TIMING", True)
    ...
    def _add_file_handler(self, log_level: int) -> None:
        ...
        When CODEX_DOCS_SOURCE_NAME is set (by parallel workers), creates
...
def get_logger(script_name: str | None = None) -> ScriptLogger:
    ...
    if _global_logger is None:
        if script_name:
            ...
            # Default logger
            _global_logger = ScriptLogger('codex-cli-docs')
