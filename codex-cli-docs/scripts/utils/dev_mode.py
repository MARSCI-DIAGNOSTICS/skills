...
# -*- coding: utf-8 -*-
"""
dev_mode.py - Development mode detection and path override for codex-cli-docs plugin.

This module provides explicit dev/prod mode separation via environment variable.
When CODEX_DOCS_DEV_ROOT is set to a valid skill directory, all paths resolve
there instead of the installed plugin location.

Mode Detection Priority:
1. Dev Mode: CODEX_DOCS_DEV_ROOT env var set to valid skill directory
...
Environment Variables:
    CODEX_DOCS_DEV_ROOT: Path to development skill directory.
...
DEV_ROOT_ENV_VAR = "CODEX_DOCS_DEV_ROOT"
...
def is_dev_mode() -> bool:
    ...
    Returns True if CODEX_DOCS_DEV_ROOT is set to a valid skill directory.
...
def get_effective_skill_dir(fallback: Path) -> Path:
    ...
    If CODEX_DOCS_DEV_ROOT is set and valid, returns that path.
