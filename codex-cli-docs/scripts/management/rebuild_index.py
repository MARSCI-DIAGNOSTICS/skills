    ...
    for md_file in md_files:
        try:
            ...
            if content.startswith('---'):
                try:
                    ...
                    if len(parts) >= 3:
                        ...
                        sitemap_url_val = frontmatter.get('sitemap_url')
                except Exception:
                    pass
            ...
            if doc_id in existing_index:
                ...
                # Fix backslash paths and detect path changes
                needs_update = False
                ...
                # Populate missing metadata fields from frontmatter
                if url and not existing.get('url'):
                    existing['url'] = url
                    needs_update = True
                if source_type and not existing.get('source_type'):
                    existing['source_type'] = source_type
                    needs_update = True
                if sitemap_url_val and not existing.get('sitemap_url'):
                    existing['sitemap_url'] = sitemap_url_val
                    needs_update = True
