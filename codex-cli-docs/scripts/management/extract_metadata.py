...
class MetadataExtractor:
    ...
    def extract_description(self, max_length: int = 200) -> str | None:
        ...
        for line in lines:
            ...
            if line.startswith('>'):
                continue
            # Skip HTML/JSX component lines (preserve HTML comments)
            if line.startswith('<') and not line.startswith('<!'):
                # Multi-line tag: skip until closing > or />
                if not (line.endswith('>') or line.endswith('/>')):
                    in_html_tag = True
                continue
            # Skip attribute lines inside multi-line HTML/JSX tags
            if in_html_tag:
    ...
    def extract_tags(self) -> list[str]:
        ...
        if 'sandbox' in body_lower:
            tags.add('sandbox')
        if 'openai' in body_lower:
            tags.add('openai')
