...
argument-hint: [describe the situation or behavior]
...
- `professional-communication` - For general email and written communication
- `/soft-skills:feedback-conversations` - Generate structured feedback using these frameworks
...
When invoked directly by the user, this skill generates structured feedback using the SBI model.

### Execution Workflow

1. **Parse Arguments** - Extract the situation or behavior description from `$ARGUMENTS`. If no arguments provided, ask the user to describe the situation.
2. **Determine Feedback Type** - Classify as positive reinforcement, constructive feedback, or mixed. Ask if needed.
3. **Apply SBI Framework** - Structure the input into Situation-Behavior-Impact components with specific, observable facts.
4. **Generate Feedback Script** - Produce a ready-to-use feedback script with:
   - Opening statement (neutral, intent-setting)
   - SBI-formatted core message
   - Dialogue prompts to encourage two-way conversation
   - Suggested follow-up actions
5. **Offer Variations** - Provide tone variations (direct, empathetic, coaching) so the user can choose the approach that fits their relationship with the recipient.
