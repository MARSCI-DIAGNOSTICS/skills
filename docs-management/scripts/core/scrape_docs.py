...
class DocScraper:
    ...
    def try_fetch_markdown(self, url: str) -> tuple[str | None, str | None, str | None]:
        ...
        try:
            ...
            # Verify content is markdown-like (not an HTML error page or SPA shell)
            ...
            # Fallback: accept non-standard content from .md endpoints if it's not
            # a SPA shell or HTML error page
            if not is_markdown:
                ...
                if not is_spa_shell and not is_html_page and len(content_stripped) > 50:
                    is_markdown = True
                    print(f"  Content accepted from .md endpoint (non-standard format, {len(content_stripped)} chars)")
    ...
    def parse_sitemap(self, sitemap_url: str, url_filter: str | None = None,
        ...
        try:
            ...
            # Apply exclude patterns (e.g., SPA-only SDK pages with no server-side content)
            if exclude_patterns:
                for ep in exclude_patterns:
                    exclude_re = re.compile(ep)
                    before = len(urls)
                    urls = [url for url in urls if not exclude_re.search(url)]
                    excluded = before - len(urls)
                    if excluded > 0:
                        print(f"  Excluded {excluded} URLs matching pattern: {ep}")
    ...
    @staticmethod
    def normalize_smart_quotes(content: str) -> str:
        """Replace typographic/smart quotes with straight ASCII quotes.

        Upstream sources (especially Anthropic blog content) use curly quotes.
        This normalizes them per project standards (CLAUDE.md: straight quotes only).
        """
        replacements = {
    ...
    def _extract_body_lines(content: str) -> list[str]:
        """Extract non-empty body lines from content, stripping YAML frontmatter.

        Used by the content regression guard to compare line counts between
        existing and incoming content.

        Args:
            content: Full file content (may include YAML frontmatter)

        Returns:
            List of non-empty body lines (after frontmatter)
        """
        lines = content.split('\n')
        body_start = 0
        if lines and lines[0].strip() == '---':
            for i in range(1, len(lines)):
                if lines[i].strip() == '---':
                    body_start = i + 1
                    break
        return [line for line in lines[body_start:] if line.strip()]
    ...
    def scrape_url(self, url: str, output_path: Path, source_type: str,
        ...
        markdown = self.normalize_smart_quotes(markdown)
        ...
        # Content regression guard: prevent overwriting large files with tiny content
        # (e.g., SPA shell "Loading..." pages replacing real docs)
        if output_path.exists():
            existing_content = output_path.read_text(encoding='utf-8')
            existing_body = self._extract_body_lines(existing_content)
            new_body = self._extract_body_lines(final_content)
            if len(existing_body) > 50 and len(new_body) < len(existing_body) * 0.10:
                print(f"  ⚠️  Regression guard: new content ({len(new_body)} lines) is <10% "
                      f"of existing ({len(existing_body)} lines), skipping overwrite: {output_path}")
                return False
    ...
    def scrape_from_sitemap(self, sitemap_url: str, output_subdir: str | None = None,
                            ...
                            expected_count: int | None = None, resume: bool = False,
        ...
        urls = self.parse_sitemap(sitemap_url, url_filter, max_age_days, exclude_patterns)
...
Examples:
  ...
  python scrape_docs.py --sitemap https://docs.claude.com/sitemap.xml \\
    ...
    parser.add_argument('--exclude-patterns', type=str, nargs='*', default=[],
                       help='Regex patterns to exclude URLs from sitemap (e.g., /api/cli/ /api/php/)')
