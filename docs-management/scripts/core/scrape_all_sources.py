...
class MultiSourceScraper:
    ...
    def scrape_source(self, source: Dict) -> Dict:
        ...
        if True:
            ...
            # Add exclude patterns for sitemap sources
            exclude_patterns = source.get('exclude_patterns', [])
            if exclude_patterns:
                cmd.extend(['--exclude-patterns'] + exclude_patterns)
                log_with_timestamp(f"   Excluding {len(exclude_patterns)} URL pattern(s)")
    ...
    def scrape_all(self, sources: list[Dict], parallel: bool = True,
        ...
        # Filter out docs-map-metadata sources (handled by enrich_categories.py, not scrape_docs.py)
        metadata_sources = [s for s in sources if s.get('type') == 'docs-map-metadata']
        if metadata_sources:
            metadata_names = [s['name'] for s in metadata_sources]
            print(f"\n⏭️  Skipping {len(metadata_sources)} enrichment source(s) (use enrich_categories.py): {', '.join(metadata_names)}")
        sources = [s for s in sources if s.get('type') != 'docs-map-metadata']
