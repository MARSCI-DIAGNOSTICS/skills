...
def rebuild_index(base_dir: Path, dry_run: bool = False) -> dict:
    ...
    for md_file in md_files:
        try:
            ...
            # Extract URL and published_at from frontmatter
            url = None
            published_at = None
            if content.startswith('---'):
                try:
            ...
            if doc_id in existing_index:
                ...
                if existing_path_normalized != relative_path_str:
                    ...
                    if published_at and existing.get('published_at') != str(published_at):
                        existing['published_at'] = str(published_at)
                    updated_entries[doc_id] = existing
                else:
                ...
                if published_at:
                    new_entry['published_at'] = str(published_at)
    ...
    if not dry_run and (existing_index or new_entries):
        ...
        # Collect all additions and updates into a single batch
        all_changes = {}
        all_changes.update(new_entries)
        all_changes.update(updated_entries)

        # Handle renames in-memory: add aliases to new entries, collect old IDs for removal
        removals = []
        for old_id, new_id in renamed_entries:
            entry = all_changes.get(new_id) or existing_index.get(new_id)
            if entry:
                # Ensure we have a mutable copy if from existing_index
                if new_id not in all_changes:
                    entry = dict(entry)
                    all_changes[new_id] = entry
            ...
            removals.append(old_id)

        # Also remove orphaned entries
        removals.extend(orphaned)

        # Single write for all additions/updates (replaces 629 individual writes)
        if all_changes:
            manager.batch_replace_entries(all_changes)

        # Single write for all removals
        if removals:
            manager.batch_remove_entries(removals)
