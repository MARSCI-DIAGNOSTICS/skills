...
# spaCy is optional -- the hardcoded ENGLISH_STOP_WORDS fallback list (326 words)
# produces identical output. spaCy is only used if already installed.
try:
...
class MetadataExtractor:
    ...
    _yake_extractor = None  # Cached YAKE KeywordExtractor instance
    _yake_config_cache: dict | None = None  # Cached YAKE config params
    _filtering_lists_cache: dict[str, set[str]] | None = None  # Cached filtering lists
    ...
    def _get_stop_words(self) -> set[str]:
        ...
        # Use comprehensive hardcoded list as primary (always available, identical
        # to spaCy's 326-word list). Merge spaCy stop words as enhancement if present.
        stop_words: set[str] = {
        ...
        # Merge spaCy stop words if available (enhancement, not required)
    ...
    def _get_filtering_lists(self) -> dict[str, set[str]]:
        """Get filtering lists from config (cached at class level)"""
        if MetadataExtractor._filtering_lists_cache is not None:
            return MetadataExtractor._filtering_lists_cache
        result = {
        ...
        MetadataExtractor._filtering_lists_cache = result
        return result
    ...
    def _extract_with_yake(self, stop_words: set[str], filters: dict[str, set[str]]) -> tuple[set[str], bool, int]:
        ...
        try:
            ...
            # Get YAKE config from defaults.yaml (cached at class level)
            if MetadataExtractor._yake_config_cache is None:
                MetadataExtractor._yake_config_cache = {
                    'language': get_default('keyword_extraction.yake', 'language', 'en'),
                    'max_ngram_size': get_default('keyword_extraction.yake', 'max_ngram_size', 3),
                    'dedup_threshold': get_default('keyword_extraction.yake', 'dedup_threshold', 0.7),
                    'top_keywords': get_default('keyword_extraction.yake', 'top_keywords', 15),
                    'min_text_length': get_default('keyword_extraction.yake', 'min_text_length', 50),
                }
            yake_cfg = MetadataExtractor._yake_config_cache
            ...
            if len(yake_text) < yake_cfg['min_text_length']:
                return keywords, yake_used, count

            # Extract keywords with YAKE using cached extractor instance
            try:
                if MetadataExtractor._yake_extractor is None:
                    MetadataExtractor._yake_extractor = yake.KeywordExtractor(
                        lan=yake_cfg['language'],
                        n=yake_cfg['max_ngram_size'],
                        dedupLim=yake_cfg['dedup_threshold'],
                        top=yake_cfg['top_keywords'],
                        features=None
                    )
                yake_keywords = MetadataExtractor._yake_extractor.extract_keywords(yake_text)
    ...
    def extract_all(self, track_stats: bool = False) -> dict:
        ...
        metadata = {
            ...
            'published_at': self.frontmatter.get('published_at'),
        }
