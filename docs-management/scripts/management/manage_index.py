...
def cmd_extract_keywords(manager: IndexManager, base_dir: Path, skip_existing: bool = True, verbose: bool = False, auto_install: bool = True, json_output: bool = False) -> None:
    ...
    print(f"   YAKE: {'✅ Available (enhanced keyword extraction)' if YAKE_AVAILABLE else '❌ Missing (using heading/content analysis)'}")
    print(f"   spaCy: {'✅ Available (stop words)' if SPACY_AVAILABLE else '⚠️  Not installed (using built-in stop words -- identical output)'}")
    print()
    if skip_existing:
    ...
    for doc_id, metadata in manager.list_entries():
        ...
        try:
            ...
            update_dict = {}
            for key in ['title', 'description', 'keywords', 'tags', 'category', 'domain', 'subsections', 'published_at']:
                if key in extracted:
