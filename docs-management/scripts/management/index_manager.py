...
class IndexManager:
    ...
    def batch_replace_entries(self, entries: dict[str, Dict]) -> bool:
        """
        Replace multiple entries in a single operation (no protected-fields merge).

        Unlike batch_update_entries which merges with existing metadata,
        this method fully replaces each entry. Use this when you have
        complete entry data (e.g., from rebuild_index.py).

        Args:
            entries: Dictionary mapping doc_id -> complete metadata dict
        ...
        # Acquire lock with retry
        if not self._acquire_lock_with_retry("batch replace"):
            return False
        ...
        try:
            ...
            # Safety check: prevent writing empty index (data loss prevention)
            if not self._validate_index_not_empty(index, "batch replace"):
                return False

            # Replace all entries in memory with progress logging
            total = len(entries)
            for i, (doc_id, metadata) in enumerate(entries.items()):
                if i > 0 and i % BATCH_PROGRESS_INTERVAL == 0:
                    pct = (i * 100) // total
                    print(f"   Batch replace progress: {i}/{total} entries ({pct}%)")
                index[doc_id] = metadata
            ...
            print(f"❌ Error in batch replace: {e}")
            return False
    ...
    def batch_remove_entries(self, doc_ids: list[str]) -> int:
        """
        Remove multiple entries in a single operation.
        ...
        # Acquire lock with retry
        if not self._acquire_lock_with_retry("batch remove"):
            return 0
        ...
        try:
            ...
            # Safety check: prevent writing empty index (data loss prevention)
            if not self._validate_index_not_empty(index, "batch remove"):
                return 0

            # Remove all matching entries in memory
            removed = 0
            ...
            print(f"❌ Error in batch remove: {e}")
            return 0
