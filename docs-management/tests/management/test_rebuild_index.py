...
class TestRebuildIndex:
    ...
    def test_rebuild_index_extracts_published_at(self, temp_dir):
        """Test that rebuild_index extracts published_at from frontmatter."""
        refs_dir = TempReferencesDir()

        try:
            # Create a markdown file with published_at in frontmatter
            refs_dir.create_doc('test', 'category', 'dated-doc.md',
                               create_mock_frontmatter(source_url='https://example.com/dated',
            ...
            # Load index and verify published_at is present
            manager = IndexManager(refs_dir.references_dir)
            entry = manager.get_entry('test-category-dated-doc')
            assert entry is not None, "Entry should exist"
            assert entry.get('published_at') == '2025-06-15', "published_at should be extracted from frontmatter"

        finally:
            refs_dir.cleanup()

    def test_rebuild_index_no_published_at_when_absent(self, temp_dir):
        """Test that rebuild_index does not add published_at when not in frontmatter."""
        refs_dir = TempReferencesDir()

        try:
            # Create a markdown file without published_at
            refs_dir.create_doc('test', 'category', 'undated-doc.md',
                               create_mock_frontmatter(source_url='https://example.com/undated') + '\n# Undated Document\n\nContent.')
            ...
            # Load index and verify published_at is NOT present
            manager = IndexManager(refs_dir.references_dir)
            entry = manager.get_entry('test-category-undated-doc')
            assert entry is not None, "Entry should exist"
            assert 'published_at' not in entry, "published_at should not be present when absent from frontmatter"
