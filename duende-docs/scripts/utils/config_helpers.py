...
def get_http_timeout() -> float:
    ...
    result = get_default('http', 'default_timeout', 60.0)
...
def get_http_user_agent() -> str:
    ...
    return get_default('http', 'user_agent', "Duende-Docs-Scraper/1.0 (Educational purposes)")

def get_management_user_agent() -> str:
    """Get user agent string for management/cleanup operations."""
    return get_default('user_agents', 'management', "Duende-Docs-Management-Bot/1.0")

def get_scraper_user_agent() -> str:
    """Get user agent string for scraping operations."""
    return get_default('user_agents', 'scraper', "Duende-Docs-Scraper/1.0 (Educational purposes)")
...
def get_output_dir_mapping(domain: str) -> str:
    ...
    Examples:
        docs.duendesoftware.com -> docs-duendesoftware-com
        www.duendesoftware.com -> duendesoftware-com
...
def get_domain_weight(domain: str) -> float:
    ...
    Args:
        domain: Domain name (e.g., "docs.duendesoftware.com")
    ...
    # Try to match by category path (e.g., docs.duendesoftware.com/identityserver)
...
def get_query_stop_words() -> set[str]:
    ...
    Example: "when to use identityserver" -> search for "identityserver" only
...
def get_domain_stop_words() -> set[str]:
    ...
    be filtered from searches (e.g., duende, identityserver).
