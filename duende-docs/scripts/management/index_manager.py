...
Usage:
    ...
    # Use config default (typically canonical/)
...
class IndexManager:
    ...
    def _check_stale_lock(self) -> bool:
        ...
        try:
            ...
            if lock_age > STALE_LOCK_THRESHOLD:
                print(f"Warning: Removing stale lock file (age: {lock_age:.1f}s > {STALE_LOCK_THRESHOLD:.0f}s threshold)")
    ...
    def _acquire_lock(self, timeout: float | None = None) -> bool:
        ...
        print(f"Warning: Lock acquisition timed out after {elapsed:.1f}s ({retry_count} retries)")
    ...
    def _acquire_lock_with_retry(self, operation: str) -> bool:
        ...
        if self._acquire_lock():
            return True
        print(f"Warning: Could not acquire index lock for {operation}, retrying...")
    ...
    def _load_yaml_chunked(self) -> Dict:
        ...
        try:
            ...
            estimated_tokens = self._estimate_file_size()
            if estimated_tokens >= TOKEN_ESTIMATE_THRESHOLD:
                print(f"Error: Chunked parsing failed for large file ({estimated_tokens} estimated tokens): {e}")
            ...
            print(f"Warning: Chunked parsing failed: {e}, falling back to full load (file is small)")
            return self._load_yaml_full()
    ...
    def _save_json(self, index: Dict) -> bool:
        ...
        try:
            ...
            print(f"Warning: Error saving index.json: {e}")
    ...
    def load_all(self) -> Dict:
        ...
        if self.json_path.exists():
            try:
                ...
                print(f"Warning: JSON load failed, falling back to YAML: {e}")
    ...
    def get_entry(self, doc_id: str) -> Dict | None:
        ...
        try:
            ...
            print(f"Warning: Error reading entry {doc_id}: {e}")
            return None
    ...
    def list_entries(self) -> Iterator[tuple[str, Dict]]:
        ...
        try:
            ...
            print(f"Warning: Error loading entries: {e}")
    ...
    def regenerate_json(self) -> bool:
        ...
        try:
            ...
            if not index:
                print("Warning: YAML index is empty, nothing to regenerate")
                return False
            ...
            if self._save_json(index):
                ...
                print("Error: Failed to save index.json")
                return False
