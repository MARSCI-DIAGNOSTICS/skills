...
class MetadataExtractor:
    ...
    def extract_tags(self) -> list[str]:
        """Extract tags for categorization - Duende-specific"""
        tags: set[str] = set()
        ...
        # Auto-categorize from path - Duende-specific
        path_str = self.file_path.as_posix().lower()

        # Duende product tags
        if 'identityserver' in path_str:
            tags.add('identityserver')
        if 'bff' in path_str:
            tags.add('bff')
        if 'accesstokenmanagement' in path_str:
            tags.add('accesstokenmanagement')
        if 'identitymodel' in path_str:
            tags.add('identitymodel')
        if 'oidcclient' in path_str:
            tags.add('oidcclient')

        # Topic tags
        if 'oauth' in path_str:
            tags.add('oauth')
        if 'oidc' in path_str or 'openid' in path_str:
            tags.add('oidc')
        if 'token' in path_str:
            tags.add('tokens')
        if 'quickstart' in path_str or 'getting-started' in path_str:
            tags.add('quickstart')
        if 'config' in path_str or 'configuration' in path_str:
            tags.add('configuration')
        if 'client' in path_str:
            tags.add('clients')
        if 'scope' in path_str:
            tags.add('scopes')
        if 'grant' in path_str:
            tags.add('grants')
        if 'dpop' in path_str:
            tags.add('dpop')
        if 'mtls' in path_str:
            tags.add('mtls')
        if 'blazor' in path_str:
            tags.add('blazor')
        if 'aspnetcore' in path_str or 'asp.net' in path_str:
            tags.add('aspnetcore')

        # Content-based tags
        body_lower = self.body.lower()
        if 'identity server' in body_lower or 'identityserver' in body_lower:
            tags.add('identityserver')
        if 'backend for frontend' in body_lower or 'bff' in body_lower:
            tags.add('bff')
    ...
    def extract_category(self) -> str | None:
        """Auto-detect category from path/content - Duende-specific"""
        if 'category' in self.frontmatter:
            return str(self.frontmatter['category']).lower().strip()

        path_str = self.file_path.as_posix().lower()

        # Duende product categories
        if 'identityserver' in path_str:
        ...
        if 'identitymodel' in path_str and 'oidcclient' in path_str:
            return 'identitymodel-oidcclient'
        if 'identitymodel' in path_str:
            return 'identitymodel'
        if 'introspection' in path_str:
        ...
        if 'reference' in path_str or 'api' in path_str:
            return 'reference'
        if 'sample' in path_str or 'example' in path_str:
            return 'samples'
        if 'upgrade' in path_str or 'migration' in path_str:
            return 'upgrades'
    ...
    def extract_domain(self) -> str | None:
        ...
        if 'duende' in path_str:
            return 'docs.duendesoftware.com'
