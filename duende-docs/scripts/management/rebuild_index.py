...
canonical from config) and rebuilds the index.yaml file. Handles:
...
def rebuild_index(base_dir: Path, dry_run: bool = False) -> dict:
    ...
    if (
        ...
        logger.warning("Existing index.yaml is non-empty but loaded as empty.")
            ...
            if content.startswith('---'):
                try:
                    ...
                    if len(parts) >= 3:
                        ...
                        # Prefer doc_id from frontmatter (set by scraper) over path-based
                        frontmatter_doc_id = frontmatter.get('doc_id', '')
                except Exception:
                    pass

            # Use doc_id from frontmatter if available, else generate from path
            if frontmatter_doc_id:
                doc_id = frontmatter_doc_id
            else:
                # Fallback: generate doc_id from path (for manually added files)
    ...
    if orphaned:
        logger.warning(f"\nOrphaned entries (file not found):")
...
def main() -> None:
    ...
    try:
        ...
        if stats['orphaned'] > 0 and not args.dry_run:
            logger.warning(f"\nWarning: {stats['orphaned']} orphaned entries found. Review and remove manually if needed.")
