...
source_url: https://docs.duendesoftware.com/identityserver/identityserver4-v31-to-identityserver4-v41/
source_type: llms-full-txt
content_hash: sha256:167f9510b23172a93958314a74a8e82cc963c021ac9d9858035654acc91b92eb
category: identityserver
doc_id: identityserver/identityserver4-v31-to-identityserver4-v41
---
...
Also, all the prior tables associated with the `ApiResources` were prefixed with "Api" and that prefix became "ApiResource" to better indicate the association. Then any new tables associated with the new top-level ApiScopes have the "ApiScope" prefix to indicate that association.

To properly update the database, the easiest approach is to first update to the latest of IdentityServer4 v4. Once that's complete, then it's straightforward to move to Duende IdentityServer v6.
...
[Section titled "Step 1: Update NuGet package to IdentityServer4 v4.x"](#step-1-update-nuget-package-to-identityserver4-v4x)
...
[Section titled "Step 2: Update Database Schema with EF Core Migrations"](#step-2-update-database-schema-with-ef-core-migrations)

If you are using a [database](/identityserver/data) for your configuration and operational data, then there is a bit of work. The reason is that for this type of schema restructuring EntityFramework Core's migrations can lose existing data. To handle this, custom SQL will perform the conversation from the old schema to the new. This is only needed for the configuration database, not the operational one so normal migrations will suffice for the operational database.
...
[Section titled "Step 3: Verify Configuration Database Data"](#step-3-verify-configuration-database-data)
...
[Section titled "Step 4: Move Onto The Upgrade Guide For Duende IdentityServer v6"](#step-4-move-onto-the-upgrade-guide-for-duende-identityserver-v6)
