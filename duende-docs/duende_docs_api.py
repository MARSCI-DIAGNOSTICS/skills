#!/usr/bin/env python3
"""
Public API for duende-docs skill.

Adapted for Duende Documentation from cursor-ecosystem pattern.

Provides a clean, stable API for external tools to interact with the
Duende documentation management system. This API abstracts away
implementation details and provides simple functions for common operations.

Usage:
    from duende_docs_api import find_document, resolve_doc_id, get_docs_by_tag

    # Find documents by query
    docs = find_document("client credentials flow")

    # Resolve doc_id to metadata
    doc = resolve_doc_id("duendesoftware-com-identityserver-overview")

    # Get docs by tag
    docs = get_docs_by_tag("oauth")
"""
...
class DuendeDocsAPI:
    ...
    Provides high-level functions for Duende documentation operations.
    ...
    def find_document(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        Args:
            query: Natural language search query (e.g., "how to configure client credentials")
        ...
        Example:
            >>> api = DuendeDocsAPI()
            >>> docs = api.find_document("client credentials flow")
            >>> print(docs[0]['title'])
        """
        try:
    ...
    def resolve_doc_id(self, doc_id: str) -> dict[str, Any] | None:
        ...
        Args:
            doc_id: Document identifier (e.g., "duendesoftware-com-identityserver-overview")
        ...
        Example:
            >>> api = DuendeDocsAPI()
            >>> doc = api.resolve_doc_id("duendesoftware-com-identityserver-overview")
            >>> print(doc['title'])
        """
        try:
    ...
    def get_docs_by_tag(self, tag: str, limit: int = 100) -> list[dict[str, Any]]:
        ...
        Args:
            tag: Tag to filter by (e.g., "oauth", "oidc", "tokens", "bff", "quickstart")
        ...
        Example:
            >>> api = DuendeDocsAPI()
            >>> docs = api.get_docs_by_tag("oauth")
            >>> print(len(docs))
        """
        try:
    ...
    def get_docs_by_category(self, category: str, limit: int = 100) -> list[dict[str, Any]]:
        ...
        Args:
            category: Category to filter by (e.g., "identityserver", "bff", "accesstokenmanagement")
            limit: Maximum number of results to return (default: 100)
        ...
        Example:
            >>> api = DuendeDocsAPI()
            >>> docs = api.get_docs_by_category("identityserver")
            >>> print(len(docs))
        """
        try:
    ...
    def search_by_keywords(self, keywords: list[str], limit: int = 25) -> list[dict[str, Any]]:
        ...
        Example:
            >>> api = DuendeDocsAPI()
            >>> docs = api.search_by_keywords(["oauth", "tokens"])
            >>> print(docs[0]['title'])
        """
        try:
    ...
    def get_document_section(self, doc_id: str, section_heading: str) -> dict[str, Any] | None:
        ...
        Example:
            >>> api = DuendeDocsAPI()
            >>> section = api.get_document_section("duendesoftware-com-identityserver-overview", "Installation")
...
_default_api: DuendeDocsAPI | None = None


def _get_api() -> DuendeDocsAPI:
    """Get or create default API instance."""
    global _default_api
    if _default_api is None:
        _default_api = DuendeDocsAPI()
    return _default_api
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Duende Docs API')
    ...
    if args.command == 'find':
        ...
        doc_id = args.args[0] if args.args else 'duendesoftware-com-identityserver-overview'
        ...
        tag = args.args[0] if args.args else 'oauth'
        ...
        category = args.args[0] if args.args else 'identityserver'
        ...
        keywords = args.args if args.args else ['oauth', 'tokens']
